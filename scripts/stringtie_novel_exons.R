...
# STRTIE <- "/Volumes/Shared/kathi/microexon_pipeline/simulation/analysis/stringtie/me_exon/outSJfilterOverhangMin6_stringtie.gtf"
# OUTFILE <- "/Volumes/Shared/kathi/microexon_pipeline/simulation/analysis/stringtie/me_exon/novel_exons_outSJfilterOverhangMin6_stringtie.txt"
...
results <- pred_tab %>% select(seqnames, lend, start, end, rstart, strand, cov) %>% rename(min_reads = cov)

## remove duplicate predictions: we take the maximum coverage for identical exon predictions
results <- results %>% group_by(seqnames, lend, start, end, rstart, strand) %>% summarise(min_reads = max(min_reads)) 
