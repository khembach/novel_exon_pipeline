...
get_transcripts <- function(pred, exons){
  ...
  ## find all exons that end in lend
    id1 <- mcols( subsetByOverlaps(exons,
                                   GRanges(seqnames = pred[1],
                                           ranges = IRanges(as.numeric(pred[2]),
                                                            as.numeric(pred[2]) ),
                                           strand = pred[6]),
                                   type = "end",
                                   ignore.strand = FALSE)
  ...
  if(!is.na(pred[5])){
    ...
    id2 <- mcols( subsetByOverlaps(exons,
                                   GRanges(seqnames = pred[1],
                                           ranges = IRanges(as.numeric(pred[5]),
                                                            as.numeric(pred[5])),
                                           strand = pred[6]),
                                   type = "start",
                                   ignore.strand = FALSE)
  ...
  if(terminal == 0){ ## it is a cassette exon
    ...
    olap_tr <- mcols( subsetByOverlaps(exons[ mcols(exons)$transcript_id %in% tr ],
                                       GRanges(seqnames = pred[1],
                                               ranges = IRanges(as.numeric(pred[2]) + 1,
                                                                as.numeric(pred[5])-1  ),
                                               strand = pred[6]),
                                       ignore.strand = FALSE)
    )$transcript_id
  ...
  ###### TODO: make sure that we only keep transcripts that start with the overlapping exons!!
    ...
    olap_tr <- mcols( subsetByOverlaps(exons[ mcols(exons)$transcript_id %in% tr ],
                                       GRanges(seqnames = pred[1],
                                               ranges = IRanges(as.numeric(pred[3]),
                                                                as.numeric(pred[5])-1 ),
                                               strand = pred[6]),
                                       ignore.strand = FALSE)
    )$transcript_id
...
new_transcript <- function(pred, tr_ids, exons){
  ...
  copy <- tr_copy[  match(paste0( tr_ids, "_", type, "_", pred$start, ":",
                                 pred$end ),
...
novel_entries <- lapply(seq_along(tr_ids), function(x)
                        if(length(tr_ids[[x]])>0){
                          new_transcript(exon_pred[x],
                                         tr_ids[[x]], exons = exons)
                          } else NA )
