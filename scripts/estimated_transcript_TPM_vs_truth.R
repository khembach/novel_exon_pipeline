...
p <- ggplot(tr_s_truth, aes(x = TPM_stringtie, y = TPM_truth)) + 
  geom_point() + 
  theme_bw() + 
  scale_x_continuous(trans="log1p", limits=c(0,max(gene_s_truth$TPM_stringtie))) + 
  scale_y_continuous(trans="log1p", limits=c(0,max(gene_s_truth$TPM_truth))) +
...
## full join, we need all genes
p <- ggplot(s_FP_FN_gene_TPM, aes(x = TPM_sum_FP, y = TPM_sum_FN)) + 
  geom_point() + 
  theme_bw() + 
  scale_x_continuous(trans="log1p", limits=c(0,max(gene_s_truth$TPM_sum_FP))) + 
  scale_y_continuous(trans="log1p", limits=c(0,max(gene_s_truth$TPM_sum_FN))) +
...
## all ids are equal except for the ones that straingtie could not match to a transcript
