...
dat_expression_long <- tidyr::gather(dat_expression, key = expression, value = recall, low, high)
...
medium_truth <- removed[ removed$class == "medium",]
hard_truth <- removed[ removed$class == "hard",]

rec_easy <- sapply(read_cutoffs, function(x) compute_rec(novelExons, easy_truth, x) )
rec_medium <- sapply(read_cutoffs, function(x) compute_rec(novelExons, medium_truth, x) )
rec_hard <- sapply(read_cutoffs, function(x) compute_rec(novelExons, hard_truth, x) )
...
## only take all exons with >10 simulated reads
removed_expressed <-removed[ removed$count_reads > 10,]
expressed_easy_truth <- removed_expressed[ removed_expressed$class == "easy",]
rec_expressed_easy <- sapply(read_cutoffs, function(x) compute_rec(novelExons, expressed_easy_truth, x) )
plot(rec_expressed_easy, pr, type = "l")

## missing predictions:
fn_expressed_easy_truth <- dplyr::anti_join(expressed_easy_truth, novelExons, by = c("seqnames", "start", "end", "strand", "lend", "rstart"))

fn_easy_truth <- dplyr::anti_join(easy_truth, novelExons, by = c("seqnames", "start", "end", "strand", "lend", "rstart"))
